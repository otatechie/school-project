...
export default function Edit({
    ...
    const { data, setData, put, processing, errors, isDirty } = useForm({
        voucher_date: voucher.voucher_date?.slice(0, 10) ?? '',
        payee_name: voucher.payee_name ?? '',
        payee_account_number: voucher.payee_account_number ?? '',
        payee_bank: voucher.payee_bank ?? '',
        payee_phone: voucher.payee_phone ?? '',
        description: voucher.description ?? '',
        amount: String(voucher.amount ?? ''),
        payment_method: voucher.payment_method ?? 'cheque',
        cheque_number: voucher.cheque_number ?? '',
        payment_reference: voucher.payment_reference ?? '',
        budget_line: voucher.budget_line ?? '',
        budget_code: voucher.budget_code ?? '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(paymentVouchers.update(voucher.id).url, {
            onError: () => {
                document
